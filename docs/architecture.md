...
Media-to-MD 采用本地单机架构：浏览器前端调用 FastAPI 后端，后端在本机文件系统中管理任务，并直接调用本机 CLI 完成转换。
...
4. runner 以 argv 列表方式启动外部 CLI。
5. 后端将 stdout/stderr 追加到 `logs/job.log` 并生成事件。
